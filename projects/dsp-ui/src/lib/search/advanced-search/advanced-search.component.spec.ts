...
describe('AdvancedSearchComponent', () => {
    ...
    describe('Ontology without resources', () => {
        beforeEach(() => {
            ...
            (dspConnSpy.v2.onto as jasmine.SpyObj<OntologiesEndpointV2>).getOntologiesMetadata.and.callFake(
                () => {
                    ...
                    const minimalOnto = new OntologyMetadata();
                    minimalOnto.id = 'minimalid';
                    minimalOnto.label = 'minimalOnto';

                    ontoMetadata.ontologies = [minimalOnto];
        ...
        it('should keep buttons disabled and show a helpful message when an ontology that contains no resources is selected', async () => {
            ...
            (dspConnSpy.v2.ontologyCache as jasmine.SpyObj<OntologyCache>).getOntology.and.callFake(
                (ontoIri: string) => {
                    ...
                    const minimalOnto = MockOntology.mockReadOntology('http://0.0.0.0:3333/ontology/0001/minimal/v2');
                    const knoraApiOnto = MockOntology.mockReadOntology('http://api.knora.org/ontology/knora-api/v2');

                    const ontoMap: Map<string, ReadOntology> = new Map();

                    ontoMap.set('http://api.knora.org/ontology/knora-api/v2', knoraApiOnto);
                    ontoMap.set('http://0.0.0.0:3333/ontology/0001/minimal/v2', minimalOnto);
            ...
            (selectOntoComp.componentInstance as TestSelectOntologyComponent).ontologySelected.emit(
                'http://0.0.0.0:3333/ontology/0001/minimal/v2'
            );

            testHostFixture.detectChanges();

            expect(testHostComponent.advancedSearch.activeOntology).toEqual('http://0.0.0.0:3333/ontology/0001/minimal/v2');
            expect(testHostComponent.advancedSearch.activeResourceClass).toEqual(undefined);
            expect(testHostComponent.advancedSearch.resourceClasses.length).toEqual(0);

            const selectResClassComp = hostCompDe.query(By.css('.no-resources-message'));

            expect(selectResClassComp.nativeElement.innerText).toEqual('This onotology contains no resources');
            ...
            expect(dspConnSpy.v2.ontologyCache.getOntology).toHaveBeenCalledTimes(1);
            expect(dspConnSpy.v2.ontologyCache.getOntology).toHaveBeenCalledWith('http://0.0.0.0:3333/ontology/0001/minimal/v2');
